...
import type { AttendanceRow } from '../core/operational/types';
...
export async function refreshOperationalSnapshot(workspaceId: string): Promise<OperationalSnapshot> {
  ...
  const [projects, profiles, teams, attendanceRows, workspaceSettingsBlob, serverMetricsResult, allocationPeriods] = await Promise.all([
