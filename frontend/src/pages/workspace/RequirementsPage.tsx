      ...
      <div className="flex-none p-6 border-b border-white/10">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">Manage client and internal project requirements.</p>
          </div>
          <button 
                ...
                <div className="flex items-start justify-between mb-3">
                  <span className={`text-[10px] uppercase tracking-wider font-mono px-2 py-1 rounded
                    ${req.priority === 'critical' ? 'bg-red-500/20 text-red-400' :
                      ...
                      req.priority === 'medium' ? 'bg-blue-500/20 text-blue-400' : 'bg-gray-500/20 text-gray-400'}`}>
                    {req.priority}
                  </span>
                  <span className={`text-[10px] uppercase tracking-wider px-2 py-1 rounded
                    ${req.status === 'Approved' ? 'bg-emerald-500/20 text-emerald-400' :
                      ...
                      req.status === 'Converted' ? 'bg-indigo-500/20 text-indigo-400' : 'bg-gray-500/20 text-gray-400'}`}>
                    {req.status}
                  </span>
                </div>
