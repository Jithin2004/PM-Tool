...
export function WorkloadBar({ label, value, maxValue = 100, status = 'healthy', onClick }: WorkloadBarProps) {
  ...
  return (
    <div
      className={`flex items-center gap-3 py-1.5 ${onClick ? 'cursor-pointer hover:bg-white/[0.02] transition-colors px-2 rounded' : ''}`}
      onClick={onClick}
    >
      <span className="text-[11px] font-mono text-white/70 w-24 truncate shrink-0">{label}</span>
