  ...
  return (
    <div className="max-w-[1600px] mx-auto px-3 sm:px-6 py-6 sm:py-12 space-y-8">
      <div>
        ...
        <p className="text-sm text-text-secondary font-mono tracking-tighter">Roles, skills, and workload across all teams</p>
      </div>

      <div className="grid grid-cols-1 gap-8">
        {enrichedTeams.map((team: any) => (
          <div key={team.id} className="border border-border bg-surface overflow-hidden">
            <div className="p-6 border-b border-border bg-bg flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 border border-border bg-white/5 flex items-center justify-center"><Users className="w-5 h-5 text-signal-info" /></div>
                <div>
                  <h3 className="text-sm font-semibold">{team.name}</h3>
                  <p className="text-[10px] font-mono text-text-tertiary">{team.members.length + (team.pm ? 1 : 0)} members</p>
                </div>
              </div>
            </div>
            {team.pm && (
              <div className="px-6 py-3 border-b border-border-subtle bg-surface-3 flex items-center gap-3">
                <Shield className="w-4 h-4 text-signal-info" />
            ...
            <div className="divide-y divide-white/5">
              {team.members.map((member: any) => {
                ...
                return (
                  <div key={member.id} className="px-6 py-4 flex items-center justify-between hover:bg-surface-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 border border-border bg-white/5 flex items-center justify-center text-[10px] font-mono">
                        {(member.full_name || member.email).charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <p className="text-xs font-medium text-text-secondary">{member.full_name || member.email}</p>
                        <p className="text-[9px] font-mono text-text-tertiary uppercase">{member.role}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-6">
                      <div className="flex items-center gap-2">
                        <Activity className="w-3 h-3 text-text-quaternary" />
                        <span className="text-[10px] font-mono text-text-tertiary">{member.openTasks} tasks</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Clock className="w-3 h-3 text-text-quaternary" />
                        <span className="text-[10px] font-mono text-text-tertiary">{member.workloadHours}h</span>
                      </div>
              ...
              {team.members.length === 0 && (
                <div className="px-6 py-8 text-center text-xs font-mono text-text-quaternary italic">No members assigned</div>
              )}
            </div>
          </div>
        ))}
