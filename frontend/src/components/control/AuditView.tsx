...
const AuditView = React.memo(function AuditView() {
  ...
  if (!wsId) {
    return <div className="flex-1 flex items-center justify-center text-white/50 font-mono text-sm">No workspace selected</div>;
  }

  return (
    <div className="max-w-[1600px] mx-auto px-3 sm:px-6 py-6 sm:py-12">
      <div className="mb-6">
        <h2 className="text-sm font-bold uppercase tracking-wider text-white">Audit Log</h2>
        <p className="text-[10px] font-mono text-white/50 uppercase">Immutable SHA-256 hash chain</p>
      </div>

      <div className="border border-white/10 bg-[#0c0c0c] p-4 mb-6">
        <h3 className="text-[10px] font-mono uppercase tracking-widest text-white/70 mb-3">Chain Health</h3>
        {loading ? (
      ...
      <div className="border border-white/10 bg-[#0c0c0c]">
        <div className="grid grid-cols-12 gap-2 px-4 py-2 border-b border-white/5 text-[9px] font-mono uppercase tracking-wider text-white/40">
        ...
        {logs.length === 0 && !loading && (
          <div className="px-4 py-8 text-center text-[11px] font-mono text-white/40">No audit logs found</div>
        )}
        {logs.map((log, i) => (
          <div key={log.id || i} className="grid grid-cols-12 gap-2 px-4 py-1.5 border-b border-white/5 text-[10px] font-mono text-white/60 hover:bg-white/5">
            ...
            <div className="col-span-2 truncate text-white/40">{log.hash?.slice(0, 16) || '-'}</div>
            <div className="col-span-2 truncate text-white/30">{log.previous_hash?.slice(0, 16) || '-'}</div>
            <div className="col-span-3 truncate text-white/40">{JSON.stringify(log.metadata).slice(0, 40)}</div>
          </div>
        ))}
      </div>
    </div>
