  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 pb-4 border-b border-white/10">
        ...
        <div>
          <h3 className="text-sm font-mono uppercase tracking-widest text-white/70">Workflow Stages</h3>
          <p className="text-[10px] text-white/40 mt-0.5">Customize your board columns and WIP limits</p>
        </div>
      </div>

      <div className="space-y-2">
        {columns.map((col, i) => (
          <div key={col.id} className="flex items-center gap-2 p-2 border border-white/10 rounded-sm bg-white/[0.02]">
            <GripVertical className="w-3.5 h-3.5 text-white/20 shrink-0" />
            <input
              value={col.title}
              onChange={e => updateColumn(i, 'title', e.target.value)}
              placeholder="Column name"
              className="flex-1 bg-transparent border-0 text-xs font-mono text-white/70 placeholder-white/20 outline-none focus:text-white"
            />
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] font-mono text-white/30 uppercase">WIP</span>
              <input
                type="number"
                min={0}
                value={col.wipLimit}
                onChange={e => updateColumn(i, 'wipLimit', parseInt(e.target.value) || 0)}
                className="w-12 bg-white/5 border border-white/10 rounded-sm px-1.5 py-1 text-[10px] font-mono text-white/70 text-center outline-none focus:border-white/20"
              />
            </div>
            {columns.length > 2 && (
              <button onClick={() => removeColumn(i)} className="text-white/20 hover:text-white/50 transition-colors">
                <X className="w-3 h-3" />
              </button>
            )}
          </div>
      ...
      <div className="flex items-center gap-3 pt-4 border-t border-white/10">
        <button
          onClick={handleSave}
          disabled={saving}
          className="px-4 py-2 bg-cyan-600 text-white text-[10px] font-mono uppercase tracking-wider hover:bg-cyan-500 transition-all rounded-sm disabled:opacity-50"
        >
