...
const INTEGRATIONS = [
  { key: 'github', label: 'GitHub', fields: ['repo_url', 'branch'], icon: 'GH' },
  { key: 'gitlab', label: 'GitLab', fields: ['repo_url', 'branch'], icon: 'GL' },
  { key: 'figma', label: 'Figma', fields: ['frame_url'], icon: 'Fi' },
  { key: 'google_drive', label: 'Google Drive', fields: [], icon: 'GD' },
];
...
export default function ProjectSettingsPanel({ projectId, onClose }: Props) {
  ...
  const [tab, setTab] = useState<'integrations'>('integrations');
  ...
  useEffect(() => {
    (async () => {
      const cfg = await fetchIntegrationConfigs(wsId, projectId);
      setConfigs(cfg);
  ...
  const handleSync = async (service: string) => {
    ...
    let result: SyncResult & { frameId?: string; title?: string };
    const cfg = editing[service] || {};
    switch (service) {
      case 'github':
        result = await syncGitHubRepo(wsId, cfg.repo_url || '', cfg.branch || 'main');
        break;
      case 'gitlab':
        result = await syncGitLabRepo(wsId, cfg.repo_url || '', cfg.branch || 'main');
        break;
      case 'figma':
        result = await syncFigmaFrame(wsId, cfg.frame_url || '');
        if (result.frameId) setFigmaPreview(`https://www.figma.com/embed?embed_host=resolve&url=${encodeURIComponent(cfg.frame_url || '')}`);
        break;
      case 'google_drive':
        result = await syncGoogleDrive(wsId);
        break;
      default:
        result = { success: false, message: 'Sync unavailable' };
    }
      ...
      <div className="w-full max-w-2xl bg-[#0c0c0c] border border-white/15 p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-4 mb-6 border-b border-white/10 pb-3">
          <button onClick={() => setTab('integrations')}
            className={`text-[10px] font-mono uppercase tracking-wider ${tab === 'integrations' ? 'text-white border-b-2 border-white/40 pb-3' : 'text-white/40 hover:text-white/60'}`}>
            Integrations
          </button>
        </div>
              ...
              return (
                <div key={svc.key} className="border border-white/10 bg-white/[0.02] p-4">
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    {(svc.key === 'github' || svc.key === 'gitlab' || svc.key === 'figma') && (
                  ...
                  {svc.key === 'figma' && figmaPreview && (
                    <div className="mt-3 border border-white/10 bg-black/50 p-2">
                      <iframe src={figmaPreview} title="Figma Preview" className="w-full h-48 border-0"
                        allowFullScreen />
                    </div>
                  )}

                  {/* Recent activity card for GitHub/GitLab */}
                  {(svc.key === 'github' || svc.key === 'gitlab') && cfg.repo_url && (
                    <div className="mt-3 border border-white/5 bg-white/[0.01] p-3">
                      <div className="text-[9px] font-mono text-white/30 uppercase tracking-wider mb-1">Recent Activity</div>
                      <div className="text-[10px] font-mono text-white/50">
                        Repository: {cfg.repo_url}<br />
