...
export function ImpactSimulationDrawer({ simulation, onApply, onDismiss, onApplyMitigation, onClose }: Props) {
  ...
  const riskColor = simulation.risk_delta > 0 ? 'text-rose-400' : simulation.risk_delta < 0 ? 'text-emerald-400' : 'text-white/50';
  const confColor = simulation.confidence_delta < 0 ? 'text-rose-400' : simulation.confidence_delta > 0 ? 'text-emerald-400' : 'text-white/50';
  const delayColor = simulation.eta_delta > 0 ? 'text-amber-400' : simulation.eta_delta < 0 ? 'text-emerald-400' : 'text-white/50';
  ...
  return (
    <div className="fixed bottom-6 right-6 w-[420px] max-h-[85vh] bg-[#0c0d14]/95 border border-white/10 rounded-xl shadow-2xl backdrop-blur-xl z-50 flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-[#07080e]/50">
        <div className="flex items-center gap-2">
          <BrainCircuit className="w-4 h-4 text-cyan-400" />
          <span className="text-[11px] font-mono font-semibold uppercase tracking-wider text-white/80">Impact Simulation</span>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white/5 rounded-lg p-3 border border-white/5">
            ...
            <div className="text-lg font-bold text-white">{simulation.affected_entities.length}</div>
          </div>
          <div className="bg-white/5 rounded-lg p-3 border border-white/5">
            <div className="flex items-center gap-1.5 text-[10px] font-mono text-white/40 uppercase mb-1">
        ...
        <div className="bg-white/5 rounded-lg p-3 border border-white/5">
          ...
          <div className="text-lg font-bold text-white">{simulation.affected_entities.length - (simulation.trigger_id ? 1 : 0)}</div>
        </div>
              ...
              {simulation.mitigations.map(m => (
                <div key={m.type} className={`flex items-center justify-between px-3 py-2 rounded-md border text-[11px] transition-all ${
                  ...
                  <div className="flex-1 min-w-0 mr-2">
                    ...
                    <div className="text-[9px] text-white/40 truncate">{m.description}</div>
                  </div>
        ...
        {showDetails && (
          <div className="bg-white/5 rounded-lg border border-white/10 divide-y divide-white/5 max-h-48 overflow-y-auto">
            {simulation.affected_entities.map(e => (
              <div key={e.taskId} className="px-3 py-2 flex items-center justify-between text-[11px]">
                <span className="text-white/70 truncate mr-2">{e.taskName}</span>
                <span className={`shrink-0 font-mono ${e.deltaDays > 0 ? 'text-amber-400' : e.deltaDays < 0 ? 'text-emerald-400' : 'text-white/30'}`}>
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-t border-white/10 bg-[#07080e]/50">
        <button onClick={handleDismiss} disabled={dismissing || isInactive} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-[10px] font-mono uppercase tracking-wider border border-white/15 text-white/50 rounded-md hover:text-white/80 hover:border-white/30 transition-all cursor-pointer disabled:opacity-40">
          <X className="w-3 h-3" />
          {isInactive ? 'Close' : 'Ignore'}
        </button>
        <button onClick={() => setShowDetails(v => !v)} className="flex items-center justify-center gap-1.5 px-3 py-2 text-[10px] font-mono uppercase tracking-wider border border-white/10 text-white/50 rounded-md hover:text-cyan-400 hover:border-cyan-500/30 transition-all cursor-pointer">
          <ArrowRight className={`w-3 h-3 transition-transform ${showDetails ? 'rotate-90' : ''}`} />
          Preview
        </button>
        <button onClick={handleApply} disabled={applying || isInactive} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-[10px] font-mono uppercase tracking-wider bg-cyan-500/15 border border-cyan-500/30 text-cyan-400 rounded-md hover:bg-cyan-500/25 transition-all cursor-pointer disabled:opacity-40">
