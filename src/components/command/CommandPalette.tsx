...
export default function CommandPalette(props: Props) {
  ...
  const allResults = useMemo((): CmdResult[] => {
    ...
    // RECENT — only show when query is empty
    if (!q) {
    ...
    const matchedNav = q ? filterItems(visibleNav, q) : visibleNav;
    ...
    // PROJECTS
    const matchedProjects = q ? filterItems(projects, q) : [];
    ...
    // TASKS
    const matchedTasks = q ? filterItems(tasks, q) : [];
    ...
    const matchedActions = q ? filterItems(visibleActions, q) : (q ? [] : []);
    ...
    // AI
    const matchedAI = q ? filterItems(AI_ITEMS, q) : (q ? [] : []);
